...
## Setup
Notifications must be manually enabled for your browser at the OS level.

### MacOS
System Settings → Notifications → Google Chrome (or other browser if preferred)
- Switch the toggle on
- Set Alert Style to "Persistent" (in case you've stepped away at the moment of the notification)

## Usage

Navigate to any Bamboo build or deploy result page. While the job is in progress, a small Notify toggle will appear in the top-left corner of the status ribbon.

The notification is off by default. Check the box to enable it before the job completes. When the job finishes, the notification will trigger.
